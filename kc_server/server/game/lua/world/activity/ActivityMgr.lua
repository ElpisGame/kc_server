...
local ActivitySixYuan = require "activity.ActivitySixYuan"
local ActivityYiYuan = require "activity.ActivityYiYuan"
...
function ActivityMgr:PlayerLogin(dbid)
	print("--------ActivityMgr:PlayerLogin---------")
	local player = server.playerCenter:GetPlayerByDBID(dbid)
	if not player then 
		print("ActivityMgr:PlayerLogin----------not player----", dbid)
		return 
	end
	local data = {}
	data.datas = {}
	...
	for activityId,activity in pairs(self.activityList) do
		print("----PlayerLogin add activity data---",activityId)
...
function ActivityMgr:InitEx()
	...
	self.activityList[ActivityConfig.ActivitySixYuan] = ActivitySixYuan.new(ActivityConfig.ActivitySixYuan)
	self.activityList[ActivityConfig.ActivitySixYuan]:AddActivity(30)

	self.activityList[ActivityConfig.ActivityYiYuan] = ActivityYiYuan.new(ActivityConfig.ActivityYiYuan)
	self.activityList[ActivityConfig.ActivityYiYuan]:AddActivity(31)
end
...
function ActivityMgr:AddRechargeCash(dbid, cash)
	...
	self.activityList[ActivityConfig.ActivitySixYuan]:AddRechargeCash(player, cash)
end
...
function ActivityMgr:onPlayerDayTimer(dbid)
	...
	self.activityList[ActivityConfig.ActivityYiYuan]:onPlayerDayTimer(dbid)
	self:ResetActDataByType(dbid, ActivityConfig.ActivityOadvance)
	self:ResetActDataByType(dbid, ActivityConfig.ActivitySixYuan)
end
...
function ActivityMgr:BuyGift(dbid, giftid)
	...
	print("----ActivityMgr:BuyGift------",giftid)
	local player = server.playerCenter:GetPlayerByDBID(dbid)
	if giftid >= ActivityConfig.YiYunGiftIdStart and giftid <= ActivityConfig.YiYunGiftIdEnd  then
		self.activityList[ActivityConfig.ActivityYiYuan]:BuyGift(player, giftid)
	elseif giftid == ActivityConfig.SixYunGiftId then
		self.activityList[ActivityConfig.ActivitySixYuan]:BuyGift(player, giftid)
	else	
