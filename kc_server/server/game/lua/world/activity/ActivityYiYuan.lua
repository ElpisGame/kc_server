...
local ActivityYiYuan = oo.class(ActivityBaseType)

function ActivityYiYuan:ctor(activityId)
end

function ActivityYiYuan:GetMyConfig(activityId)
	return ActCfg:GetActConfig("ActivityType31Config", activityId)
end

function ActivityYiYuan:PackData(player, activityId)
...
function ActivityYiYuan:onPlayerDayTimer(dbid)
...
function ActivityYiYuan:BuyGift(player, giftid)
...
function ActivityYiYuan:Reward(dbid, index, activityId)
	local player = server.playerCenter:GetPlayerByDBID(dbid)
	local activity = self.activityListByType[activityId]
	local record = server.activityMgr:GetActData(player, activityId)
	...
	player.activityPlug:GiveReward(activityId, record, table.wcopy(rewardCfg.item), "1元礼包")
	self:SendActivityDataOne(player, activityId)

	server.serverCenter:SendLocalMod("logic", "chatCenter", "ChatLink", 48, nil, nil, player.cache.name(), rewardCfg.name, ItemConfig:ConverLinkText(rewardCfg.item[1]))
end

return ActivityYiYuan
