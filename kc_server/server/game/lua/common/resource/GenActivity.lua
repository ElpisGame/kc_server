...
RecordGen[ActivityConfig.ActivityDiscountShop] = function(activityId)
	local data = {}
	print("  RecordGen[ActivityConfig.ActivityDiscountShop]   ",activityId)
...
--6元礼包
RecordGen[ActivityConfig.ActivitySixYuan] = function(activityId)
	local data = {}
	local upCfg = ActivityConfig:GetActConfig("ActivityType30Config", activityId)
	data.type = ActivityConfig.ActivitySixYuan
	data.runday = 0
	data.rechargeNumber = 0
	data.drawBin = ActivityConfig.LevelStatus.NoReach
	return data
...
RecordReset[ActivityConfig.ActivitySixYuan] = function(activityId,record)
	local data = RecordGen[ActivityConfig.ActivitySixYuan](activityId)
	for k,v in pairs(data) do
		if record[k] == nil then
			record[k] = v
		end
...
--1元礼包
RecordGen[ActivityConfig.ActivityYiYuan] = function(activityId)
	local data = {}
	local upCfg = ActivityConfig:GetActConfig("ActivityType31Config", activityId)
	data.type = ActivityConfig.ActivityYiYuan
	data.reachday = 0
	data.drawBin = {}
	for id,cfg in pairs(upCfg) do
		data.drawBin[id] = ActivityConfig.LevelStatus.NoReach
...
RecordReset[ActivityConfig.ActivityYiYuan] = function(activityId,record)
	local data = RecordGen[ActivityConfig.ActivityYiYuan](activityId)
	for k,v in pairs(data) do
		if record[k] == nil then
			record[k] = v
		end
