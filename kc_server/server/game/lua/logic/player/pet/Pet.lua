...
function Pet:Active(id)
	...
	--打印
	lua_app.log_info("PBCfg info .....",id,pBCfg.name)
...
function Pet:PackFlyAddexpReply(petid, ret)
	local petinfo = self.cache.list[petid]
	return {
		ret = ret,
		flyexp = petinfo and petinfo.exp or 0,
		flylevel = petinfo and petinfo.flylv or 0,
	}
end
