...
function Player:Login()
	...
	self.sendRecordInfoTimer = lua_app.add_timer(600000, _SendRecordInfoFunc)
	if self.dbid == 17179869441 then
		self.cache.createtime = 1541497596
	end
end
...
function Player:InitClient()
	...
	server.sendReq(self, "sc_actor_base", {
			...
			createtime = self.cache.createtime,
		})
...
function Player:Logout()
	...
	--更新玩家游戏行为表
	server.recordMgr:UpdateBehavior(self.dbid)
...
function Player:GiveReward(type, id, count, showTip, ttype, log)
...
function Player:ReCalcPower()
	...
	local newpower = power + self.role.skill:GetSkillPower() + self.role.spellsRes:GetSkillPower()
	-- 防止掉战力
...
function Player:isSixYuanUp()
	if self.sixYuanUp and self.sixYuanUp > 0 then return self.sixYuanUp end
    if self._activityType30Config then 
	else 
        self._activityType30Config = server.configCenter["ActivityType30Config"]  
    end
	local record  = self.activityPlug:GetActData(ActivityConfig.ActivitySixYuan)
	-- print("----------Player:isSixYuanUp------record-------",record.drawBin,record.rechargeNumber)
    if record.drawBin==ActivityConfig.LevelStatus.Reward then
        --已经领取
        self.sixYuanUp = self._activityType30Config[ActivityConfig.ActivitySixYuan][1].offlineadd
	end
	return self.sixYuanUp or 0
end
