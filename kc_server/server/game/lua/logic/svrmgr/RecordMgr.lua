...
local behaviortbname = server.GetSqlName("behavior")
local ordersbname = server.GetSqlName("orders")

-- local loginname = server.GetSqlName("login")



local RecordMgr = {}

--这里可能要初始化一个定时器，然后进行后台命令的处理
function RecordMgr:Init()
	self.timerID = lua_app.add_update_timer(1000, self, "RunTime")	 	--从数据库读取玩家充值数据（发货）add by tang
	self.gmtimerID = lua_app.add_update_timer(1000, self, "RunGmCmd")	--从数据库读取并处理GM命令
	...
	server.mysqlBlob:CreateDmg(behaviortbname)
	server.mysqlBlob:CreateDmg(ordersbname)

	-- server.mysqlBlob:CreateDmg(loginname)
...
function RecordMgr:UpdateBehavior(playerid)
	local player = server.playerCenter:DoGetPlayerByDBID(playerid)
	if not player then
		lua_app.log_error("RecordMgr:UpdateBehavior: no account:", playerid)
		return false
	end
	--更新表
	local time = os.time({year =lua_app.year(), month = lua_app.month(), day =lua_app.day(), hour =00, min =00, sec = 00})

	-- lua_app.log_info("RecordMgr:UpdateBehavior: logOutTime:", time)


	--充值数据
	local orders = server.mysqlCenter:query(ordersbname, {dbid = player.dbid,serverid = server.serverid ,complete =1 })
	local rechargeData = ''

	if orders[1] then
		for _, value in ipairs(orders) do
			rechargeData = rechargeData..","..value.goodsid..","..value.amount
		end
	end


	local rechargeCash 	= player.cache.recharge
	local playerLv 		= player.cache.level
	local allExp	 	= player.cache.exp
	local mainTaskId 	= player.cache.task.tasks[server.taskConfig.TaskType.Main]
	local guardLv 		= player.role.fairy.cache.lv
	local shenbingLv 	= player.role.weapon.cache.lv
	local mountLv 		= player.role.ride.cache.lv
	local wingLv 		= player.role.wing.cache.lv

	for k,v in pairs(mainTaskId) do
		print(k,v)
	end

    --self.role.cache.equips_data.equipList[self.slot] --装备部位

	--获取强化等级（最小等级）
	local function _getForgeLv(player,forgeType)
		local lv = 1
		local levellist = player.role.equip:GetForgeLevelList(forgeType)
		for _, level in ipairs(levellist) do
			if lv == 1 then
				lv = level
			else
				lv = math.min(level,lv)
			end
		end
		return lv
	end
	...
	--锻造
	local strengthenLv			= _getForgeLv(player,ItemConfig.ForgeType.Enhance)
	local refineLv 				= _getForgeLv(player,ItemConfig.ForgeType.Refine)
	local exerciseLv 			= _getForgeLv(player,ItemConfig.ForgeType.Anneal)
	local gemstoneLv 			= _getForgeLv(player,ItemConfig.ForgeType.Gem)
	
	--灵童
	local soulboyLv				= player.baby.babyPlug.cache.lv
	
	--仙侣
	local xianlvFazhenLv 		= player.xianlv.circle.cache.lv
	local xianlvXianweiLv		= player.xianlv.position.cache.lv
	
	--玄女
	local xuannvLv				= player.tiannv.cache.lv
	local xuannvHuanianLv   	= player.tiannv.flower.cache.lv
	local xuannvAuraLv			= player.tiannv.nimbus.cache.lv

	--宠物
	local petTonglingLv			= player.pet.soul.cache.lv
	local petShouhunLv			= player.pet.psychic.cache.lv

	--藏宝图
	local mapNum				= player.treasuremap:MaxClear()
	--玲珑宝塔层数
	local linglongbaotaNum		= player.tiannv.nimbus.cache.lv
	--永闯天庭层数
	local yongchuangtiantingNum	= player.cache.heavenFb.layer
	...
	local cacheData = {
		...
		guardLv 					= guardLv,
		shenbingLv 					= shenbingLv,
		mountLv 					= mountLv,
		wingLv 						= wingLv,
		strengthenLv  				= strengthenLv,
		refineLv 					= refineLv,
		exerciseLv					= exerciseLv,
		gemstoneLv					= gemstoneLv,


		soulboyLv					= soulboyLv,

		xianlvFazhenLv 				= xianlvFazhenLv,
		xianlvXianweiLv				= xianlvXianweiLv,
		
		xuannvLv					= xianlvXianweiLv,
		xuannvHuanianLv   			= xuannvHuanianLv,
		xuannvAuraLv				= xuannvAuraLv,
	
		petTonglingLv				= petTonglingLv,
		petShouhunLv				= petShouhunLv,
	
		mapNum						= mapNum,
		linglongbaotaNum			= linglongbaotaNum,
		yongchuangtiantingNum		= yongchuangtiantingNum,
	}

	-- lua_app.log_info("RecordMgr:UpdateBehavior: mainTaskId:"..mainTaskId)


	local value = server.mysqlCenter:query(behaviortbname, {dbid = player.dbid,serverid = server.serverid ,curTime =time })

	if value[1] then
		lua_app.log_info("RecordMgr:UpdateBehavior: 更新:")
		--更新
		server.mysqlCenter:update(behaviortbname, { dbid = player.dbid, serverid = server.serverid , curTime = time}, cacheData)
	else
		lua_app.log_info("RecordMgr:UpdateBehavior: 插入:")

		--插入
		local data = {
			dbid 						= player.dbid,
			serverid 					= server.serverid,
			account						= player.cache.name,
			curTime 					= time,
		}
		...
		server.mysqlCenter:insert(behaviortbname,data)
	end
end
...
function RecordMgr:GmCmd(cmd, param1, param2, param3, param4, param5, param6, param7)
	lua_app.log_info("--RecordMgr:GmCmd--", cmd, param1,param2)
	...
	elseif cmd == "notice" then --游戏内公告 by thb 2021/5/25
		self:notice(param1,tonumber(param2))
	end
end
...
--游戏内公告 by thb 2021/5/25
function RecordMgr:notice(content,type)
	-- body
	server.noticeCenter:NoticeStr(content,type)	
end
