...
SERVER_CONFIG="configure.xml"            #游戏服务
CENTER_CONFIG="configure_center.xml"    #中心服务
CROSS_CONFIG="configure_cross.xml"        #跨服服务
PLAT_CONFIG="configure_plat.xml"        #平台登录服务
RECORD_CONFIG="configure_record.xml"    #后台数据服务
...
dump_sql() {
    get_svrsqlinfo ${1}
    mkdir -p "/server/data"
    mysqldump --opt -u${user} -p${pass} -h${ip} -P${port} ${dbname} | gzip > "/server/data/"${dbname}.sql.gz
}
backup_sql() {
    get_svrsqlinfo ${1}
    mkdir -p "/server/data"
    mysqldump --opt -u${user} -p${pass} -h${ip} -P${port} ${dbname} | gzip > "/server/data/"${dbname}_`date +%F_%H-%M`.sql.gz
}
recover_sql() {
    get_config ${1}
...
case $TARGET in
    ...
    restartbase)    stop ${SERVER_CONFIG} 9
                    stop ${CROSS_CONFIG} 9
                    stop ${CENTER_CONFIG} 9:x
